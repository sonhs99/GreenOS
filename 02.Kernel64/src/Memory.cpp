...
void kInitializeDynamicMemory() {
    ...
    gs_stDynamicMemory.pstBitmapOfLevel = 
        (Bitmap*)(DYNAMICMEMORY_START_ADDRESS + (sizeof(u8) * gs_stDynamicMemory.iMaxLevelCount));
    u8* pbCurrentBitmapPosition =
        ((u8*) gs_stDynamicMemory.pstBitmapOfLevel) + (sizeof(Bitmap) * gs_stDynamicMemory.iMaxLevelCount);
    ...
    for(int j = 0; j < gs_stDynamicMemory.iMaxLevelCount; j++) {
        ...
        for(i = 0; i < iBlockCountLevel / 8; i++) *(pbCurrentBitmapPosition++) = 0x00;
...
void* kAllocateMemory(u64 qwSize) {
    u64 qwAlignedSize = kGetBudddyBlockSize(qwSize);
...
static bool kFreeBuddyBlock(int iBlockListIndex, int iBlockOffset) {
    ...
    for(int i = 0; i < gs_stDynamicMemory.iMaxLevelCount; i++) {
        ...
        if((iBlockListIndex % 2) == 0) iBuddyBlockOffset = iBlockOffset + 1;
...
void kGetDynamicMemoryInformation(
    ...
    pqwDynamicMemoryTotalSize = kCalculatedDynamicMemorySize();
    pqwMetaDataSize = kCalculatedMetaBlockCount(pqwDynamicMemoryTotalSize) * DYNAMICMEMORY_MIN_SIZE;
